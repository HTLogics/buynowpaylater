									...
									<tbody>
										...
										@if(count($cart_items)>0)
											...
											<tr id="row{{$cart_item->product_id}}">
												...
												@endphp
												<td><input type="hidden" name=product_id[] value="{{$cart_item->product_id}}"/>{{$item_data->name}}</td>
												...
												@endphp
												<td>{{number_format($cart_item->qty * $item_data->price,2)}}<a href="{{route('admin.remove_cart_item',['id' => $cart_item->product_id, 'cartid' => $cart->id])}}" class="round remove-item" data-url="{{route('admin.remove_cart_item',['id' => $cart_item->product_id, 'cartid' => $cart->id])}}" data-rowid="#row{{$cart_item->product_id}}"><i class="la la-times"></i></a></td>
											</tr>
											@endforeach
										@else
											<tr>
